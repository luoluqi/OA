...
export default {
  ...
  methods: {
    ...
    getDetail () {
      ...
      console.log(this.$parent.detailRow)
    },
    // 流程记录表格
    recordOperateLogByFlowRecordId () {
      let params = {
        flowRecordId: this.$parent.detailRow.Id
      }
      this.$store
        .dispatch('processCenter/RecordOperateLogByFlowRecordId', params)
        .then(res => {
    ...
    recordNodeByFlowRecordId () {
      ...
      this.$store
        .dispatch('processCenter/RecordNodeByFlowRecordId', params)
        .then(res => {
          let result = JSON.parse(res)
          // console.log(result)
          if (result.Code == 200) {
    ...
    itemByRecordIdFormRecord () {
      let params = {
        recordId: this.$parent.detailRow.FormRecordId
      }
      this.$store
