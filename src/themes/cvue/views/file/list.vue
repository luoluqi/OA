        ...
        <el-row :gutter="20" style="height:100%;">
             <el-col :span="6"  style="height:100%;">
                 <el-card  style="height:100%;overflow:scroll;">
                     ...
                     :data="folder" 
                     :props="{
                          children: 'childrenPersonalfolder',
                          label: 'label'
                     }"
                    ...
                    <cvue-table
                        ...
                        <template slot-scope="scope" slot="attach">
                          <div v-for="(item,index) in scope.row.attach" :key="index">
                             ...
                             :href="item.AttachmentFileUrl | imgUrl" target="_blank">{{item.AttachmentFileName}}</a>
                          </div>
            ...
            <el-form label-width="150px">
                <el-form-item label="排序号">
                 ...
                 <el-form-item label="文件夹名称">
                    <el-input v-model="folderName" maxlength="30"></el-input>
                </el-form-item>
            </el-form>
...
import {getFolderList, addFolder, editFolder, delFolder, getFileList, delFile} from '@/api/personFileApi'
 ...
 export default {
    ...
    methods: {
      ...
      renderContent(h, { node, data, store }) {
        ...
        if(data.personalfolder.LevelCode > 0){
          a = (
...
white-space: nowrap;">{node.label}</span>
      ...
      addFolder () {
        if(!this.folderName){
          ...
          addFolder({
                _OwnerUserId: this.cookiesObj.loginUserId,
                _Name:  this.folderName,
                _ParentId: this.parentFolder.personalfolder.Id,
                _LevelCode: this.parentFolder.personalfolder.LevelCode + 1,
                _OrderNum:  this.orderNum
            }).then(d => {
                d = JSON.parse(d)
                if(d.Code == "200"){
                  this.newFolderId = d.Data
      ...
      toEdit (e,data) {
        e.stopPropagation()
        this.folderId = data.personalfolder.Id
        this.folderName = data.personalfolder.Name
        this.orderNum = data.personalfolder.OrderNum
      ...
      toRemove (e,data){
        ...
        if(data.personalfolder.LevelCode == 0){
         ...
         this.$confirm('确定删除 “ '+data.personalfolder.Name+' ” ?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          delFolder(data.personalfolder.Id).then(d => {
             d = JSON.parse(d)
                if(d.Code == "200"){
                  if(this.curFolder.Id == data.personalfolder.Id){
                    this.handleList()
                    this.curFolder.Name = ''
                  }

                  this.isParentDeleted(data.childrenPersonalfolder)

                  this.removeAfter(this.folder,data.personalfolder.Id)
                    this.$message({
                      type: 'success',
                      message: '删除成功!'
                    })
      ...
      removeAfter (arr,id) {
        for(var i = 0;i<arr.length;i++){
          if(arr[i].personalfolder.Id == id){
            arr.splice(i,1)
          }else{
            this.removeAfter(arr[i].childrenPersonalfolder,id)
          }
        }
      },
      isParentDeleted(folderList){
        for(var i = 0;i<folderList.length;i++){
          if(folderList[i].personalfolder.Id == this.curFolder.Id){
            this.tableData = []
            this.curFolder = ''
          }else{
            this.isParentDeleted(folderList[i].childrenPersonalfolder)
          }
        }
    ...
    confirmDialog (params) {
     ...
     handleNodeClick(data) {
          console.log(data)
          this.handleList(data.personalfolder.Id)
          this.curFolder = data.personalfolder
      },
      // 改变pagesize
    ...
    handleList (id) {
      ...
      var params = {
        ownerUserId: this.cookiesObj.loginUserId,
      }
      if(this.keyword){
      ...
      getFileList(params).then(res => {
        ...
        if (result.Code == 200) {
          ...
          for(var item of list){
            item.personalfile.attach = item.personalfileattachmentList
            this.tableData.push(item.personalfile)
          }
      ...
      createLabel(list){
            for(var item of list){
                item.label = item.personalfolder.Name
                this.createLabel(item.childrenPersonalfolder)
            }
      }
    },
    mounted () {
       ...
       this.getFolderData().then(() => {
          this.handleList(this.folder[0].personalfolder.Id)
          this.curFolder = this.folder[0].personalfolder
       })
    }
  }
</script>
