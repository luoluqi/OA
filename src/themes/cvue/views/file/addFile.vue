...
import {addFile, editFile, uploadFile, uploadAttachment, delAttachment} from '@/api/personFileApi'
...
export default {
  ...
  methods: {
        ...
        if(this.type == "add"){
            ...
            for(var item of curFile.attach){
              this.attachList.push({
                ...
                contentLength:item.AttachmentFileSize,
    ...
    handleClose () {
      ...
      this.$store.state.file.personUpload.abort()
    },
    closeDialog () {
    ...
    confirmDialog () {
     ...
     if(this.type == "add"){
         addFile({
          ...
          _FolderId:this.$store.state.file.folderId,
          _Name:this.fileName,
    ...
    uploadAttach(id){
        ...
        for(var item of this.attachList){
          aList.push({
            _PersonalFileId:id,
            _AttachmentFileName:item.fileName,
            _AttachmentFileUrl:item.fileUrl,
            _AttachmentFileSize:item.contentLength
          })
        }
        if(aList.length == 0){
