        ...
        <el-tabs type="border-card" class="tabs-card">
          <div class="information">
            ...
            >
              ...
              <el-form-item label="通知公告内容:" class="postContent">
                <el-upload
                  class="upload-demo"
                  :action="actionUrl"
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        title: [
          { min: 0, max: 300, message: '标题不能超过 300 个字符', trigger: 'blur' }
        ]
        // postContent: [
  ...
  methods: {
    ...
    confirmDialog () {
      ...
      this.fileList.forEach((item, index) => {
        str += 'http://' + JSON.parse(item.response).Data.fileUrl + ','
      })
      let noticefiles = str.substring(0, str.length - 1)
      let str2 = ''
      this.fileList.forEach((item, index) => {
        str2 += item.name + ','
      })
      let noticeFileNames = str2.substring(0, str2.length - 1)
      ...
      this.$refs.ruleForm.validate((valid) => {
        ...
        let params = {
          ...
          typeCode: 0,
          announcer: this.$parent.cookiesObj.loginUserId,
        ...
        this.$store.dispatch('notice/AddNotice', paramsNew).then(res => {
          let result = JSON.parse(res)
          // console.log(result)
          if (result.Code === 200) {
            this.$message.success('添加通告成功')
